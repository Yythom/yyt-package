...
import { invariant, getRequiredChunkKey } from '@yyt/react-lazy'
...
function assetToLinkTag(asset, extraProps) {
    ...
    return `<link ${hint}="${asset.chunk}" rel="${asset.linkType}" as="${asset.scriptType
    ...
    getChunkAssets(chunks) {
        const one = chunk => {
            ...
            return chunkGroup.assets
                .map(filename => this.createChunkAsset({
                    filename,
                    chunk,
                    type: 'mainAsset',
                    linkType: 'preload',
