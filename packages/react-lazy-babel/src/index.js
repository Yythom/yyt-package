...
// const chunkNameProperty = require('./properties/chunkName')
// const importAsyncProperty = require('./properties/importAsync')
...
const loadablePlugin = api => {
    ...
    function hasLoadableComment(path) {
        ...
        const comment = comments.find(
            ({ node }) => node && node.value && String(node.value).includes(LOADABLE_COMMENT),
        )
    ...
    function transformImport(path) {
        ...
        const object = t.objectExpression(
            propertyFactories.map(getProperty => getProperty({ path, callPath, funcPath })),
        )
